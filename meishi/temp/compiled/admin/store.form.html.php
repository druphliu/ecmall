...
<?php endif; ?>
    ...
    //区域校验
    var $chosen_choices = $(".chosen-choices");
    var $seller_area_notice = $(".seller_area_notice");
    var $chosen_select = $(".chosen-select");
    $('#form_submit').click(function () {
        var area = $chosen_select.val();
        if(!area){
            $chosen_choices.addClass("error").find("input").focus();
            $seller_area_notice.addClass("error").html("此项不允许为空");
            return false
        }
    });
    $chosen_select.change(function () {
        var area = $chosen_select.val();
        if (area) {
            $chosen_choices.removeClass("error");
            $seller_area_notice.removeClass("error");
        } else {
            $chosen_choices.removeClass("error").addClass("error").find("input").focus();
            $seller_area_notice.removeClass("error").addClass("error").html("此项不允许为空");
        }
    });
    $chosen_choices.find("input").blur(function(){
        if(!$chosen_select.val()){
            $seller_area_notice.removeClass("error").addClass("error").html("此项不允许为空");
        }
    })
});
      ...
      <tr>
        ...
        <tr>
            ...
            <td>
                ...
                <select name="seller_area[]" data-placeholder="选择销售区域" style="width:350px;" class="chosen-select" multiple tabindex="6">
                    <?php $_from = $this->_var['options']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'option');if (count($_from)):
    foreach ($_from AS $this->_var['option']):
?>
                    <?php echo $this->_var['option']; ?>
                    <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
                </select>
                <label generated="true" class="seller_area_notice">销售区域,必填</label>
      ...
      <tr>
        <th></th>
        <td class="ptb20"><input class="formbtn" type="submit" id="form_submit" name="Submit" value="提交" />
