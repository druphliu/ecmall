...
?>
                <tr class="line<?php if (($this->_foreach['v']['iteration'] == $this->_foreach['v']['total'])): ?> last_line<?php endif; ?>">
                    ...
                    <td><?php if ($this->_var['coupon']['min_amount']): ?><?php echo sprintf('一次订餐满 %s', $this->_var['coupon']['min_amount']); ?><?php else: ?>不限制<?php endif; ?></td>
                    <td><?php if ($this->_var['coupon']['use_area']): ?><?php echo $this->_var['coupon']['use_area_name']; ?><?php else: ?>无限制<?php endif; ?></td>
                    <td class="align2">
                        <?php if ($this->_var['coupon']['if_issue'] && $this->_var['coupon']['end_time'] > $this->_var['time']): ?>
                        <a href="javascript:void(0);" class="log" uri="index.php?app=coupon&act=make&id=<?php echo $this->_var['coupon']['coupon_id']; ?>" dialog_id="coupon_make" dialog_title="生成优惠码" dialog_width="320" ectype="dialog">生成优惠码</a>
                        <a href="javascript:void(0);" class="extend" uri="index.php?app=coupon&act=extend&id=<?php echo $this->_var['coupon']['coupon_id']; ?>" dialog_id="coupon_extend" dialog_title="发放" dialog_width="320" ectype="dialog">发放</a>
                        <a href="index.php?app=coupon&act=sn_list&id=<?php echo $this->_var['coupon']['coupon_id']; ?>" title="优惠码列表" class="view_order">优惠码列表</a>
                        <?php elseif ($this->_var['coupon']['if_issue'] && $this->_var['coupon']['end_time'] <= $this->_var['time']): ?>
                        <a class="delete" href="javascript:drop_confirm('您确定要删除它吗？', 'index.php?app=coupon&act=drop&id=<?php echo $this->_var['coupon']['coupon_id']; ?>');">删除</a>
                        <a href="index.php?app=coupon&act=sn_list&id=<?php echo $this->_var['coupon']['coupon_id']; ?>" title="优惠码列表" class="view_order">优惠码列表</a>
                        <?php else: ?>
                        <a class="start" href="javascript:drop_confirm('一旦发布将不能修改优惠券信息', 'index.php?app=coupon&act=issue&id=<?php echo $this->_var['coupon']['coupon_id']; ?>');">发布</a>
                        <a href="javascript:void(0);" class="edit1" uri="index.php?app=coupon&amp;act=edit&id=<?php echo $this->_var['coupon']['coupon_id']; ?>" dialog_id="coupon_edit" dialog_title="编辑" dialog_width="460" ectype="dialog">编辑</a>
                        <a class="delete" href="javascript:drop_confirm('您确定要删除它吗？', 'index.php?app=coupon&act=drop&id=<?php echo $this->_var['coupon']['coupon_id']; ?>');">删除</a>
                        <?php endif; ?>
                    </td>
                </tr>
